...
<%
	...
	ArrayList<Agencia> lista = (ArrayList<Agencia>) request.getAttribute("agencias");
%>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body>
	...
	<div id="centerbar">
		<h1>Cadastro de Agências</h1>
	</div>
	<div id="main1">
	<a href="novaagencia.html" class="Botao1">Cadastrar Agência</a>
	<a href="reportAgencia" class="Botao2">Relatório</a>
	<table id="tabela">
		<thead>
			<tr>
				<th>ID</th>
		...
		<tbody>
			<%for (int i=0; i<lista.size(); i++) { %>
				<tr>
					<td><%=lista.get(i).getIdAgencia() %></td>
					<td><%=lista.get(i).getCnpj() %></td>
					<td><%=lista.get(i).getEndereco() %></td>
					<td><%=lista.get(i).getTelefone() %></td>
					<td><%=lista.get(i).getEmail() %></td>
					<td><a href="selectAgencia?IdAgencia=<%=lista.get(i).getIdAgencia() %>" class="Botao1">Editar</a>
					<a href="javascript: confirmarExclusaoAgencia(<%=lista.get(i).getIdAgencia() %>)" class="Botao2">Excluir</a>
					</td>
				</tr>
			<%} %>
		</tbody>
	...
	<script src="scripts/confirmaExclusaoAgencia.js"></script>
</body>
</html>